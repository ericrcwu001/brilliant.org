...
- **Lesson Factory — `concept/bayes-rule` worktree (Dept 3, Coder B done, commit `fdc63b2`):** `BayesUpdateBeat.tsx` renderer (3 sub-displays: bars/tree/sequence; DOM/SVG; all values via `bayes.ts`; hero/graded wiring; 44px a11y; aria-live mirrors; reduced-motion support) + dispatcher wiring in `beats/index.tsx` + 3 lesson fixtures (`lesson-bayes-rule-{1,2,3}.json`, schema-valid) + `course-bayes-rule.json` enriched (`status:"live"`, `chapters[]`, 3 lesson nodes). Renderer test: 30 passing pure model tests. `tsc -b` clean; `validate-fixtures` green (all schema passes; bayes cross-check + gate-set additions pending the parallel coder). **NOT done by Dept 3 Coder B (other coder's work):** `scripts/validate-fixtures.ts` GATED/MASTERY/cross-check additions; `interviews/` folder.
