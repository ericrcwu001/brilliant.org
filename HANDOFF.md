...
- **Built:** Flagship **L1 only** (Groups A & B complete + hardened). Group C Firebase
...
- **L1 inclusive redesign (full two-track) — DONE in the working tree** (per
  ...
  **L0** `lesson-first-heads` (optional course node); per-lesson `phases.ts`; mastery signal
  ...
  terminal) so Firestore picks up the updated L1 fixture + new L0 lesson/course node.
...
- **Last verified green** (this session): `validate` (+`E[H]=2` golden), `vitest run`
  (98 app + 7 functions), `lint`, `build`, `e2e` (9/9: smoke + Track-B `lesson-complete` +
  new Track-A pass, each across chromium / mobile / reduced-motion). Functions `tsc` via the
  local `functions/node_modules/.bin/tsc` (root tsc 6.0 only warns on the node10 config).
  e2e browsers: `playwright install chromium` then run e2e with `required_permissions:["all"]`
  (the sandbox reports x64 and redirects the browser cache, so a plain run can't find them).
...
## Building New Lessons (L2–L6) — design-only, with blockers
...
> **Ordering conflict to reconcile before building:** code/fixtures/milestones were
> rewired to **Overlap-last** (L4 States, L5 Longer, L6 Overlap = capstone), but the
> plan files + `future_ideas.md` use **Overlap = L4**. The table below follows the plan
> filenames.
...
2. **One-automaton assumption** — `LessonPlayer` builds one automaton from
   `patternOptions[0]`; graded beats only see that pattern; empty `patternOptions`
   crashes `buildAutomaton`. Multi-pattern lessons (L2 race, L6 transfer) need a
   per-beat `pattern` field + active-pattern infra.
...
6. **`CoinSimBeat` crashes on `H`** — **DONE** (guarded empty `overlapHighlights` in
   `startReplay`/`stepReplay`; L0 uses it). **`RecapBeat` is still HH/HT-hardcoded** — not
   generalized; L0 deliberately avoids it (closes on an `mcq`, not `recap`). A multi-pattern
   lesson still needs RecapBeat generalized.
...
0. **L1 inclusive redesign follow-ups:** (a) **re-seed** Firestore (emulator + prod) so the
   updated L1 fixture + new L0 lesson/course node go live; (b) browser-check Track A vs B on
   `/dev/lesson` and `/dev/lesson?track=A`; (c) the `derived.mastered` flag is persisted but
   **not yet consumed** — wire the L4 mixed review to re-surface `failure-edge` when
   `mastered=false`; (d) **validity:** test the scaffolds with real novices (spec §12.5) —
   treat high reveal/hint rates as a load-spike signal; (e) commit (working tree is large).
...
4. **Before building L2–L6:** clear the prerequisite blockers above; reconcile the
   Overlap ordering conflict.
