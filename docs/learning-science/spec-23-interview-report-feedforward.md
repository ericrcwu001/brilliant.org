...
**Privacy — most-sensitive inference (README §4.6).** The calibration/overconfidence delta is the **most sensitive personal inference** this plan surfaces. It is rendered **self-only**: shown **only** in the learner's own report, in-app, and is **never exported, shared, or sent to recruiters/instructors/other learners**, and is **never a gate** (D11 removed the verdict). It carries no share/export/copy affordance — do not add a "share report" or "download" control that would carry the calibration section off-device. This mirrors the cross-spec **owner-only / self-only-use** posture in README §4.6 (the per-attempt `calibration` block and the `calibration/summary` overconfidence trend are owner-only read and drive only the learner's own report).
...
// Interview reports are not per-concept ⇒ no conceptProgress arg; helper falls back to defaultTrack ?? 'A' (fail GENTLE).
const showCalibration = isQuantIntensity(userDoc)
```

The helper's contract (README §4): `effectiveTrack = conceptProgress?.track ?? userDoc?.defaultTrack ?? 'A'` (**fail GENTLE**), then `track === 'B' || userDoc?.learningGoal === 'interview'`. No new gate field, no `uid` lookup, no `loadTrack` call (D13: intensity-gating reuses existing `learningGoal`/track). `userDoc` may be `null` (not yet onboarded) ⇒ the helper short-circuits its `userDoc?.…` reads, and the **default-`'A'` fallback** means a `null`/track-less userDoc yields `false` — calibration is simply hidden for a not-yet-onboarded learner, which is the desired behavior and is simpler (no reliance on a second guard to hide it). Pass through the `null` userDoc as-is. The `calibration && calibration.n > 0` render guard remains as belt-and-suspenders. The dev harness (`DevInterviewPage.tsx`) renders under the same `AuthProvider`, so `useAuth()` resolves there too; if it mounts without a `userDoc`, calibration is simply hidden (and the §7 `/dev` check passes a mock `calibration` + `showCalibration` directly into `InterviewReportView` to exercise the shown path).
...
- **self-only / no export (README §4.6)** — assert `InterviewReportView` renders **no** share/export/download/copy affordance for the report or calibration section: `expect(html).not.toMatch(/share|export|download/i)` and the component exposes no callback prop that emits the `calibration` payload outward.
...
- **Privacy (README §4.6):** the calibration/overconfidence delta is rendered self-only — the report UI exposes no share/export/download/copy affordance for it (asserted in §7); it is never exported, shared, or used as a gate.
