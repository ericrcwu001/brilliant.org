# Scorecard: Correlation ρ: The Unit-Free Score  (lesson-covariance-5)

| # | gate | owner | status | evidence |
|---|------|-------|--------|----------|
| 1 | Source fidelity | Dept 1 | ✅ | every problem cited+sourced — GB p.48 (#4,#5,#7); see source-dossier.md + brief.md |
| 2 | Math correctness | Dept 3 Verify | ✅ | engine reproduces win ρ=4/5; explore ρ²=1/2; mastery ρ²=1/2 (ρ=1/√2 display-only, no float graded), Cov(X,X²)=0→ρ=0; covariance.test.ts + lesson-covariance-5.factcheck.test.ts green; validate-fixtures §3f cross-check green |
...
| 9 | Inclusivity gate | Dept 3 Verify | ✅ | validate-fixtures inclusivity + mastery-challenge gates pass for lesson-covariance-5 (added to GATED + MASTERY_LESSONS — non-vacuous) |
